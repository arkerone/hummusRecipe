...
exports._writeInfo = function _writeInfo() {
  ...
  if (!this.isNewPDF) {
    ...
    oldInfo = copyCtx
      ...
      .queryDictionaryObject(copyCtx.getSourceDocumentParser().getTrailer(), 'Info')
      .toJSObject();
  }
  ...
  const ignores = ['CreationDate', 'Creator', 'ModDate', 'Producer'];
    ...
    const iterateObjectTypes = inObject => {
      ...
      switch (type) {
        ...
        case hummus.ePDFObjectArray:
          const jsArray = inObject.toPDFArray().toJSArray();
        ...
        case hummus.ePDFObjectDictionary:
          ...
          const keys = Object.getOwnPropertyNames(aDictionary).join(', ');
          output += `- ${label} {${keys}}`;
          logToFile(output);
          ++mTabLevel;
...
exports._parseObjectByType = function _parseObjectByType(inObject) {
  ...
  switch (type) {
    ...
    case hummus.ePDFObjectArray:
      ...
      break;
    case hummus.ePDFObjectDictionary:
      const dictionaryObject = inObject.toPDFDictionary().toJSObject();
      Object.getOwnPropertyNames(dictionaryObject).forEach(element => {
        this._parseObjectByType(dictionaryObject[element]);
      });
      break;
    case hummus.ePDFObjectStream:
      const dictionary = inObject.toPDFStream().getDictionary();
